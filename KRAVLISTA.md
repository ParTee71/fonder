...
>
> Version: 0.21.6 · Paket: `se.partee71.fonder` · Språk: Svenska
...
| TP-12 | Diagram med **Vico** (`com.patrykandpatrick.vico:compose-m3`), wrappat i delad `FundLineChart` (`ui/diagram/`) — resten av appen rör aldrig Vico-API:t direkt (regel 4). Zoomar som standard in till den senaste månaden och skrollar till slutet, så den senaste kursen syns direkt (issue #47) — hela historiken nås ändå genom att nypa ut/dra i diagrammet, Vicos inbyggda pinch/pan (`Zoom.max(Zoom.Content, Zoom.x(30.0))`/`Scroll.Absolute.End`, kortare historik än en månad visar allt i stället för en tom yta). Y-axeln pads runt kursens eget min/max (`PriceRangeProvider`, issue #49) i stället för Vicos standard som alltid tvingar in noll — annars klämdes en fonds faktiska rörelse ihop mot en avlägsen nollinje. |
