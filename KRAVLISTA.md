...
| TP-6 | Inloggning via **Firebase Auth + Google Credential Manager** (issue #106). Inställningar har ett **Google-konto**-kort: utloggad visas "Inte inloggad" och en inloggningsknapp, inloggad visas mejladressen och "Logga ut". Kontoväljaren är `GetSignInWithGoogleOption` (helsida), **inte** `GetGoogleIdOption` (bottenark) — den senare faller med "Failed to retrieve an ID token" när ingen inloggning redan är cachad, alltså just vid första inloggningen. Googles ID-token växlas mot en Firebase-session, som Firebase persisterar själv; appen sparar ingenting eget. `AuthRepository` lämnar ut en egen `AuthUser`, aldrig Firebases `FirebaseUser` — annars går varken ViewModel eller UI att testa utan Firebase på klassvägen. Inloggat läge läses alltid ur `currentUser` (en `AuthStateListener` bakom `callbackFlow`), aldrig ur en lokal spegling av vad appen själv nyss gjorde: sessionen kan upphöra utan att appen bett om det (utgången token, kontot borttaget på enheten), och en spegling hade fortsatt visa en användare som inte finns. Fel returneras som `SignInException` med uppräknad orsak (`CANCELLED`/`NO_CREDENTIAL`/`FAILED`), samma form som `BackupFormatException.Reason` — och ett **avbrutet kontoval visas aldrig som ett fel**, det var ett val användaren gjorde med flit. Felet är en engångshändelse som kvitteras bort, inte ett bestående tillstånd (samma lärdom som issue #78). Utloggningen rensar Credential Managers cachade val **före** Firebase-utloggningen; utan det kommer nästa inloggning tillbaka med samma konto utan att fråga, vilket ser ut som att utloggningen inte tog. Inloggningen bär **ingen** användardata i sig — den är förutsättningen för molnbackupen (TP-7 steg 2), vilket kortets text säger rakt ut så att "inloggad" inte förväxlas med "säkerhetskopierad". Ingår därför inte i backup-kontraktet (NFR-1): en session hör till enheten, inte till portföljen. Kräver `app/google-services.json` (incheckad) — uppsättningen i Firebase/Google Cloud är dokumenterad i [docs/GOOGLE-SETUP.md](docs/GOOGLE-SETUP.md). |
| TP-7 | Backup av all användardata, i två steg. **Steg 1 (byggt, SET-6):** export/återställning mot en användarvald **lokal fil** via SAF (Storage Access Framework) — versionerat JSON-format (`BackupPayload`/`BackupSerializer`), ingen inloggning, inget nätverk, och därmed en rundtur som går att köra i CI. **Steg 2 (byggt, SET-7, issue #110):** samma sträng skriven till **Google Drive (`appDataFolder`)** — ett rent transportbyte bakom `BackupRepository`, formatet oförändrat. `DriveBackupRepository` är medvetet **inte** en andra `BackupRepository`: Drive behöver lista, hämta senaste och en auktoriseringsväg som kräver UI, och det hör inte hemma i formatets kontrakt. Anroparen komponerar de två (`export()` → `upload()`, `downloadLatest()` → `restore()`), vilket är hela skälet att kontraktet är en **sträng**. Kräver en OAuth-klient med Drive API och scope `drive.appdata`; scopet begärs vid körning via `Identity.getAuthorizationClient` **skilt från inloggningen** och först när backupen används — inloggad (TP-6) räcker alltså inte, och en användare som bara loggat in blir aldrig tillfrågad om Drive-åtkomst. |
...
| UI-11 | Varje skärm som visar **hämtad** data uppdateras med **dra ned** (`PullToRefreshContainer`, `ui/components/`, regel 4 — skärmarna rör aldrig Material3:s `PullToRefreshBox` direkt, samma gräns som TP-12 drar mot Vico): Hem och Portfölj forcerar en kursuppdatering (TP-17), Fonddetalj hämtar om fondens kurs och historik, Pågående byte kandidaternas kurvor (ANA-11/ANA-12), Facit startar bytesplansskanningen (HEM-8) och Fondsök hämtar om katalogen. Varje skärm anropar **samma kodväg** som sin egen skärmöppning, aldrig en egen variant: en manuell uppdatering ska hämta exakt det öppningen hämtar. Indikatorn drivs av samma körstatus som kortens väntesnurror (NAV-6, `BackgroundWork`) och aldrig av en lokal flagga — en lokal flagga slutar antingen snurra innan datan landat eller fortsätter efter att jobbet tagit slut. Upprepade dragningar ger aldrig parallella körningar: WorkManager koalescerar (`REPLACE` för kursuppdateringen, `KEEP` för skanningarna) och skärmarnas egna hämtningar återanvänder ett pågående jobb i stället för att avbryta det. Skärmar **utan** hämtad data (Transaktioner, Sålda fonder, Riskprofil, Inställningar, importflödena) har ingen dragning — en gest som inte gör något är värre än ingen gest — och inte heller tomt-tillstånden på Hem, Portfölj och Facit, där det per definition inte finns något att hämta. Fondsöks fel- och tomlägen är däremot svepbara (egen skrollbar yta), eftersom ett nätverksfel är just det läge man mest vill dra sig ur. **Gesten är enda vägen** till en forcerad uppdatering sedan SET-2:s knapp ströks; det är ett medvetet val och en känd avvikelse från UI-8:s princip att en gest aldrig får vara enda vägen — den som inte kan svepa är hänvisad till bakgrundsjobben. |
...
| SET-2 | Inställningar visar ett **kursuppdateringskort** med "Senast uppdaterad: \<tidsstämpel\>" (eller "Aldrig uppdaterad"). ~~och en **"Uppdatera nu"-knapp** som forcerar en kursuppdatering oavsett staleness-gate (TP-17, issue #27) — bypassar launch-gate/backstopens "bara om inaktuellt"-princip, för den som inte vill vänta.~~ *(knappen borttagen, issue #119 — den forcerade uppdateringen finns kvar men startas med **dra ned** på Hem och Portfölj, UI-11: uppdatering hör hemma där datan visas, inte tre skärmar bort under en inställningsrubrik)*. Kortet svarar alltså numera bara på "hur färsk är datan", vilket är en annan fråga än "uppdatera den". |
...
| SET-7 | Säkerhetskopieringskortet (SET-6) har även **molnbackup till Google Drive** (TP-7 steg 2, issue #110): en dygnsvis bakgrundskörning skriver hela backup-kontraktet till Drives dolda `appDataFolder`, plus knappar för att köra och återställa på begäran. Kortet visar senaste lyckade körning, och en rad när Drive-scopet saknas. **Inloggning är inte säkerhetskopiering** — kontokortets text (TP-6) säger det rakt ut, och `drive.appdata` begärs separat, först när backupen faktiskt används. Körningen ligger under ett **eget unikt arbetsnamn** (`fonder_drive_backup`) med en egen worker, inte påhängd kursuppdateringen: den manuella "Uppdatera nu" (SET-2) ersätter (`REPLACE`) allt som väntar under sitt namn och skulle annars kunna avbryta en säkerhetskopiering mitt i. Utloggad hoppas körningen över **tyst** (`Result.success`) — inte inloggad är ett läge appen ska tåla, inte ett fel; saknad auktorisering flaggas i stället för att köas om, eftersom bara användaren kan lösa den. `appDataFolder` är per Google Cloud-**projekt**, inte per app, och Fonder delar projektet med Dagboken — filnamnen prefixas därför `fonder-backup-` och **varje** listning filtrerar på prefixet. Urvalet för gallringen (de fem senaste behålls) ligger i `DriveBackupFiles` (`domain/usecase/`, ren och enhetstestad) och filtrerar om på prefixet en gång till: en obefiltrerad gallring skulle radera *den andra appens* säkerhetskopior utan väg tillbaka, och den logiken får inte ligga inbäddad bland Drive-anrop som inte går att köra i CI. `trashed = false` behövs eftersom papperskorgade filer ligger kvar i mappen och annars räknas som den senaste kopian. Återställning från Drive **ersätter**, precis som från fil, och ligger bakom samma bekräftelsedialog. Senaste körningens tidsstämpel och auktoriseringsflaggan är cache-metadata som hör till enheten och ingår medvetet **inte** i backup-kontraktet (NFR-1), samma kategori som `lastPriceSyncEpochMillis`. |
...
| ID | Krav |
|----|------|
| NFR-1 | All persisterad användardata ska överleva en **backup → restore-rundtur** utan förlust. Sedan issue #114 omfattar kontraktet även pågående och avslutade byten (`switch_watches`/`switch_watch_candidates`, ANA-12): säljdagen, beloppet, uppsättningen bevakade alternativ, deras ankrade nollpunkt (`navAtStart`) och vad som faktiskt köptes går inte att räkna fram ur NAV-historiken i efterhand — kandidaterna ligger per definition inte i kurscachen (ANA-11). Rundturen är sedan SET-6 **app-styrd och testad**: `BackupPayload` definierar kontraktet, och `BackupRoundTripTest` kör backup → SET-1-tömning → restore mot riktig Room och riktig DataStore. Ett fält som läggs till i en kontraktsbärande modell utan att komma med i formatet fäller `BackupSerializerTest`s fältvakt, i stället för att tyst falla ur varje framtida säkerhetskopia. Androids **Auto Backup** (`allowBackup="true"`) ligger kvar som komplement — den täcker en förlorad enhet utan att användaren gjort något, men är inte en rundtur appen kan verifiera. Sedan TP-6 pekar manifestet ut sina regelfiler (`fullBackupContent` för API 30, `dataExtractionRules` för 31+); `backup_rules.xml` fanns men refererades aldrig, så Auto Backup tog dessförinnan med appens **hela** datakatalog. Det var ofarligt med en inloggningsstub men inte med en persisterad Firebase-session, som varken ska till molnet eller till en ny enhet. Listorna är **include**-listor (`fonder.db` + `datastore`), så skyddet är att sessionen inte står med — inte att den exkluderas. Sedan SET-7 finns rundturen även mot molnet, och den är då **automatisk** — vilket Auto Backup aldrig kunde ersätta, eftersom appen varken kan utlösa eller verifiera den. |
| NFR-2 | Ingen beteendeändring utan **tester** på berörd nivå (enhet/instrument/migrering). CI bygger sedan issue #112 dessutom **release** (`assembleRelease`) på varje PR, parallellt med enhetstesterna: debugbygget kör inte R8, så minifiering, resursskrympning och proguard-reglerna prövades tidigare först vid ett faktiskt släpp — och brast då två gånger (#108, #111). "Grönt" omfattar därmed även att appen går att bygga i den variant användaren installerar. |
...
| SLD-5 | Ett sälj-kort med **känt ISIN** erbjuder, i sitt utfällda läge (SLD-4), att starta en bevakning av ett **pågående byte** (ANA-12) — den fristående vägen in, för ett byte gjort på eget initiativ utanför bytesplanen (HEM-8). Säljdatum och likvid (belopp − avgift) tas ur säljtransaktionen, inte ur dagens datum: utvecklingen ska mätas från den dag pengarna faktiskt frigjordes. Målnivån för de automatiska alternativen (ANA-13) är säljfondens **egen** risknivå ur metadatacachen — saknas den startas bevakningen ändå, men utan förslag; en gissad nivå hade gett alternativ på fel risk (ANA-4-principen), och inget nätverksanrop görs för att fylla luckan. Ett sälj som redan har en öppen bevakning (samma fond **och** samma säljdag) erbjuder ingen ny; utan ISIN erbjuds ingenting alls, eftersom kandidaternas kurser hämtas på ISIN. Knappen ligger i den utfällda delen — hopfällt läge är enligt SLD-4 avsiktligt bara fondnamn och resultat (issue #114). |
...
| ANA-12 | Ett fondbyte är två handlingar med dagar emellan: säljen går ut, likviden landar T+1–T+3, och först då fattas köpbeslutet. Skärmen **Pågående byte** täcker den perioden (issue #114). Den startas antingen ur ett **kvitterat** bytesförslag (HEM-8/ANA-10 — först efter kvitteringen; erbjuds den innan bytet gjorts mäter den dagar då pengarna låg kvar) eller fristående från en säljrad (SLD-5), och bär säljfond, säljdag, belopp och "dag N av `WATCH_TTL_DAYS`". Skärmen **leder med jämförelsen**: säljfonden och samtliga bevakade alternativ i **ett** diagram (delade `FundLineChart`, regel 4 — samma flerserie-läge som ANA-11, indexerat till 100 vid periodens första gemensamma dag), följt av en rad per alternativ med namn, risknivå (`RiskBadge`, UI-10), avgift och det tal beslutet hänger på: **utvecklingen sedan säljdagen** i kr och % via delade `PeriodRow`. Nollpunkten är kandidatens NAV på säljdagen, ankrad en gång när historiken hämtas och därefter sparad (`navAtStart`) — den kan inte återskapas i efterhand, eftersom kandidaten per definition inte ligger i kurscachen (ANA-11). Saknas kurs den dagen ankras **första kända dagen efter** (aldrig före: en kurs från dagen innan hade mätt en utveckling som delvis skedde i den sålda fonden) och raden märks som delvis. Kandidathistoriken hämtas vid skärmöppning och hålls **bara i minnet**, aldrig i kurscachen (ANA-11); en kandidat vars historik inte går att hämta visas utan utveckling och med orsaken utskriven, aldrig som 0 % (ANA-4-principen), och försvinner aldrig ur listan. Appen genomför **fortfarande aldrig ett byte**: "Köpte den här" registrerar vad användaren gjort, stänger bevakningen med den köpta fonden och sätter `followed` på källförslaget **bara** när det faktiskt var den föreslagna fonden som köptes — facit (SET-5) mäter följda råd, och ett byte till något annat är inte rådet som följdes. En bevakning som legat orörd längre än `WATCH_TTL_DAYS` (14 dygn — ett byte går på T+1–T+3 och tar med marginal aldrig två veckor) stängs som **utgången** av bakgrundskörningen; en stängd bevakning **raderas aldrig**, den bär vad användaren faktiskt gjorde och ingår därför i backup-kontraktet (NFR-1). |
| ANA-13 | Bevakningens alternativ (ANA-12) kommer från två håll: appen fyller listan **en gång** med de bäst rangordnade köpkandidaterna på bevakningens målnivå — samma pool och samma rangordning som bytesplanens köpkandidat (`FundMetadataRepository.findSwitchCandidates`, HEM-8: översta kvartilen på 12-månadersavkastning, därefter lägst avgift), högst `AUTO_CANDIDATES` (3) — och användaren kan lägga till egna ur Fondsök upp till taket `MAX_CANDIDATES` (5). Taket är både en kostnads- och en beslutsgräns: varje kandidat är ett historikanrop mot en odokumenterad källa (TP-14) varje gång skärmen öppnas, och tio fonder är ingen jämförelse utan en ny sökning (samma beteenderesonemang som `MAX_SWITCHES_PER_PLAN`). Utan känd målnivå hämtas **inga** förslag alls — en gissad nivå hade gett alternativ på fel risk (ANA-4-principen) — och omfyllning sker aldrig, eftersom den både kostat en källfråga per gång och kunnat byta ut de alternativ användaren just tittade på. En fond som redan bevakas i samma byte, eller säljfonden själv, erbjuds aldrig. Fondsöks katalogträffar saknar ofta ISIN (TP-21); går det inte att slå upp sägs det ut i stället för att en rad läggs till som aldrig kan visa någon utveckling. Bara **handplockade** alternativ går att ta bort — att plocka bort ett av appens förslag hade sett ut som ett val men bara dolt vad planen faktiskt föreslår. |
---

## Följdkrav (planerade — se GitHub-issues)

Inga öppna följdkrav i backup-kedjan: Google-inloggningen (TP-6) byggdes i issue #106 och
molnbackupen (TP-7 steg 2, SET-7) i issue #110. Konsolstegen för båda — Firebase-projekt,
...
- **Uppdatering låg tre skärmar bort (#119):** Ny kravrad **UI-11** (dra ned för att uppdatera på
  de sex skärmar som visar hämtad data), **SET-2**:s "Uppdatera nu"-knapp struken och **TP-17**
  omskriven — den forcerade körningen finns kvar, den startas bara där datan syns i stället för
  under en inställningsrubrik. Delad `PullToRefreshContainer` (regel 4) och ett `refresh()` per
  ViewModel som återanvänder skärmöppningens egen kodväg. Ingen ny persisterad data.
...
- **Bytet slutade där rådet gavs (#114):** Nya rader **ANA-12** (skärmen Pågående byte),
  **ANA-13** (kandidaturvalet), **HEM-11** (kortet på Hem) och **SLD-5** (fristående start ur
  en säljrad); **HEM-8**, **ANA-10** och **NFR-1** utökade. Room 14→15 med två nya tabeller,
  `switch_watches` och `switch_watch_candidates`, båda i backup-kontraktet.

  1. **Ett byte är två handlingar, inte en.** Appen kunde föreslå ett byte (HEM-8) och ta emot
     kvitteringen att det gjorts (SET-5) — men däremellan ligger T+1–T+3 dagar då likviden inte
     är framme och köpbeslutet ännu inte är fattat. För den perioden fanns ingen yta alls, och
     rådet var prissatt mot den dag det gavs.
  2. **Ett förslag är en fond, beslutet gäller flera.** Bytesplanens köpkandidat är *en* fond.
     I praktiken vägs ett par likvärdiga mot varandra innan likviden binds. Kandidaterna kommer
     därför ur samma pool och samma rangordning som planens egen (ANA-13) men visas som ett urval
     att välja **ur**, kompletterat med användarens egna, med ett tak på fem.
  3. **Nollpunkten kunde inte skjutas upp.** Utvecklingen ska mätas från säljdagen, men en
     kandidat är en fond appen aldrig ägt och ligger inte i kurscachen (ANA-11) — kursen just den
     dagen går inte att hämta i efterhand. Den ankras därför en gång vid första hämtningen och
     sparas (`navAtStart`), vilket är hela skälet till att bevakningarna är användardata och
     ingår i backup-kontraktet i stället för att vara härledd cache.
  4. **Kvitteringen får inte påstå mer än den vet.** Ett köp av en *annan* fond än den föreslagna
     sätter inte `followed` på förslaget: facit mäter följda råd, och ett byte till något annat är
     inte rådet som följdes. Bevakningen bär `boughtIsin`, så utfallet går att mäta senare — men
     att spela in det som ett eget råd i facit är ett eget issue, inte en bieffekt av det här.
  5. **Ett gammalt läge är fel på ett sätt gammal cache inte är.** Samma princip som
     `PLAN_TTL_DAYS`: en bevakning som legat orörd i mer än 14 dygn beskriver inte längre något
     som finns, och stängs som utgången av bakgrundskörningen. Den raderas aldrig — den bär vad
